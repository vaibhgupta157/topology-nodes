...
public class IPAddressAllocatorStaticTest {
    private static Logger LOGGER = Logger.getLogger(IPAddressAllocatorStaticTest.class);
    ...
    @ServiceCallback(servicePoint="ipa-static-test-servicepoint",
    ...
    public Properties create(ServiceContext context,
        ...
        LOGGER.info("IPAddressAllocatorStaticTest Servicepoint is triggered");
        ...
        try {
            ...
            try {
                ...
                NavuLeaf startIp = service.leaf("subnet-start-ip");
                NavuLeaf size = service.leaf("subnet-size");
                if (!size.exists() || !startIp.exists()) {
                    return opaque;
                }
                String subnetStartIp = service.leaf("subnet-start-ip").valueAsString();
                int subnetSize = (int)((ConfUInt8)service.leaf("subnet-size").value()).longValue();
                IPAddressAllocator.subnetRequest(service, "mypool",
                                                 "admin", subnetStartIp, subnetSize, "statictest", false);
                boolean error = false;

                boolean allocated = sess.exists(servicePath + "/allocated");
                boolean ready =
                    IPAddressAllocator.responseReady(service.context(), cdb, "mypool", "statictest");

                if (ready) {
                    try {
                        IPAddressAllocator.fromRead(cdb, "mypool", "statictest");
