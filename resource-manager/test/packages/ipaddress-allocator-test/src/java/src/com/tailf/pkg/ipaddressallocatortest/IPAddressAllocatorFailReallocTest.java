...
public class IPAddressAllocatorFailReallocTest {
    private static Logger LOGGER = Logger.getLogger(IPAddressAllocatorFailReallocTest.class);
    ...
    @ServiceCallback(servicePoint="ipa-fail-realloc-test-servicepoint",
    ...
    public Properties create(ServiceContext context,
        ...
        LOGGER.info("IPAddressAllocatorFailedReallocTest Servicepoint is triggered");
            ...
            try {
                ...
                if (ready && !error) {
                    if (!allocated) {
                        ...
                        // Test part 2: if we get here, try to access a bad path, to trigger a failed redeploy
                        sess.setElem(new ConfUInt32(5), "/devices/device{nodevice}/config/test");
                    }
                } else {
                    if (allocated) {
