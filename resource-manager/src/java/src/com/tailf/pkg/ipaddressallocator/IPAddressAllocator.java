...
public class IPAddressAllocator implements ApplicationComponent {
    ...
    private void loadState() throws NavuException, ConfException,
        ...
        NavuContainer resources =
            root.container(resourceAllocator._resource_pools_);
        NavuList ipaddressPool =
            resources.list(ipaddressAllocator._ip_address_pool_);
    ...
    private void allocateAddress(HashSet<ToRedeploy> redeps,
        ...
        boolean poolIsIpv6 = req.pool.ipPool.isIpv6();

        String owner = "";
        String requestId = "";
        String username = "";
        RedeployType redeployType = RedeployType.DEFAULT;

        String redeployTypeString = req.path + "/" + ipaddressAllocator._redeploy_type_;
        ...
        String subnetStartIpPath = req.path + "/" +
                ipaddressAllocator._request_ + "/" +
                ipaddressAllocator._subnet_start_ip_;
        ...
        if (maapi.exists(tid, redeployTypeString)) {
            ConfEnumeration rtEnum = (ConfEnumeration) maapi.getElem(tid, redeployTypeString);
            String rt = ConfValue.getStringByValue(redeployTypeString, rtEnum);
            redeployType = RedeployType.from(rt);
        }
        ...
        int requestCidr = subnetSize;
        if(invertSubnetSize) {
            if(!poolIsIpv6) {
                requestCidr = (32 - subnetSize) < 0 ? 0 : (32 - subnetSize);
            } else {
                requestCidr = (128 - subnetSize) < 0 ? 0 : (128 - subnetSize);
            }
        }

        InetAddress requestAddress = null;
        if(maapi.exists(tid, subnetStartIpPath)) {
            String subnetStartIp =
                maapi.getElem(tid, subnetStartIpPath).toString();
                requestAddress = InetAddress.getByName(subnetStartIp);
        } else {
            if(!poolIsIpv6) {
                requestAddress = InetAddress.getByName("0.0.0.0"); // ANY IPv4 address
            } else {
                requestAddress = InetAddress.getByName("::"); // ANY IPv6 address
            }
        }
        ...
        try {
            Subnet requestSubnet = new Subnet(requestAddress, requestCidr);
            if(!requestAddress.toString().equals(requestSubnet.getAddress().toString())){
                LOGGER.debug("Invalid address/cidr pair.");
                throw new InvalidNetmaskException("Invalid address/cidr pair.");
            }

            a = req.pool.ipPool.allocate(requestSubnet, owner, username, requestId);
            ...
            wsess.setCase(ipaddressAllocator.
                          ...
                          req.path+"/"+
                          ipaddressAllocator._response_);
        } catch (InvalidNetmaskException ex) {
            wsess.setElem(new ConfBuf(ex.toString()),
                        req.path + "/"+
                        ipaddressAllocator._response_+"/"+
                        ipaddressAllocator._error_);
        ...
        if (owner != "") {
             ...
             */
            ToRedeploy t = new ToRedeploy(owner, username, redeployType);
    ...
    private void createPool(NavuContainer pool)
        ...
        NavuList poolSubnet = pool.list(ipaddressAllocator._subnet_);
        NavuList poolRange = pool.list(ipaddressAllocator._range_);
            ...
            /* Convert range request to subnets; add to subnets */
            for (NavuContainer range : poolRange.elements()) {
                String from = range.leaf(ipaddressAllocator._from_).
                        value().toString();
                String to = range.leaf(ipaddressAllocator._to_).
                        value().toString();
                InetAddress addr1 = InetAddress.getByName(from);
                InetAddress addr2 = InetAddress.getByName(to);
                InetAddressRange rng = new InetAddressRange(addr1, addr2);
                for (Subnet rsub : rng.getSubnets()) {
                    subnets.add(rsub);
                }
            }
            ...
            /* Add request subnets to availables */
            for (NavuContainer subnet : poolSubnet.elements()) {
                String address = subnet.leaf(ipaddressAllocator._address_).
                    value().toString();
                String mask = subnet.leaf(ipaddressAllocator._cidrmask_).
            ...
            /* Convert range request to subnets then add to availables
	     * We need at least one 'availables' subnet so that isIpv4()
	     * or isIpV6() work.
	     */
            for (NavuContainer range : poolRange.elements()) {
                String from = range.leaf(ipaddressAllocator._from_).
                ...
                InetAddressRange rng = new InetAddressRange(addr1, addr2);
                for (Subnet rsub : rng.getSubnets()) {
                    String address = rsub.getAddress().getHostAddress();
                    String mask = String.valueOf(rsub.getCIDRMask());

                    String subnetPath = cdbSubnetPath+"{"+address+" "+mask+"}";

                    if (!wsess.exists(subnetPath)) {
                        ipPool.addToAvailable(rsub);
                        wsess.create(subnetPath);
                    }
                }
            }
            ...
            NavuList poolExclude = pool.list(ipaddressAllocator._exclude_);
        ...
        public DiffIterateResultFlag iterate(
            ...
            try {
                ...
                if (op == DiffIterateOperFlag.MOP_CREATED &&
                    ...
                    /* this is to avoid alocating the same request more than once, for each
                     * request YANG leaf that has VALUE_SET, besides subnet_size */
                    if (!kp[0].toString().equals(ipaddressAllocator.prefix + ":"
                                               + ipaddressAllocator._subnet_size_)) {
                        return DiffIterateResultFlag.ITER_CONTINUE;
                    }
     ...
     *                  service node. Make sure that this is the node you get
     *                  in your service create!
     * @param redeployType service redeploy option ('default', 'touch',
    ...
    public static void subnetRequest(NavuNode service,
        ...
        try {
            ...
            v.putQuoted("SERVICE",
                        ...
                        .toString().replace("'", "\""));
            v.putQuoted("REDEPLOY_TYPE", redeployType.getToken());
            v.putQuoted("SUBNET_START_IP", "");
            v.putQuoted("SUBNET_SIZE", Integer.toString(cidrmask));
            v.putQuoted("INVERT", (invertCidr ? "true" : ""));
    ...
    /**
     * Create a static IP subnet allocation request. Make sure that the
     * <code>NavuNode</code> service is the same node you get in your
     * service create. This ensures that the backpointers are updated
     * correctly and that the Reactive Fastmap algorithm works as
     * intended.
     ...
     * @param username  username to use when redeploying the requesting service
     * @param cidrmask  CIDR mask length of requested subnet
     * @param startIp   Start ip for the requested subnet.
     * @param id        unique allocation id
     * @throws ResourceErrorException if the pool does not exist
     */
    public static void subnetRequest(NavuNode service,
    ...
    public static void subnetRequest(NavuNode service,
        ...
        try {
            ...
            v.putQuoted("SERVICE",
                        ...
                        .toString().replace("'", "\""));
            v.putQuoted("REDEPLOY_TYPE", redeployType.getToken());
            v.putQuoted("SUBNET_START_IP", startIp);
            v.putQuoted("SUBNET_SIZE", Integer.toString(cidrmask));
            v.putQuoted("INVERT", (invertCidr ? "true" : ""));
            t.apply(new NavuContainer(service.context()), v);
        } catch (Exception e) {
    ...
    /**
     * Create a static IP subnet allocation request. Make sure you use the
     * service context you get in the service create callback. This
     * method takes any <code>NavuNode</code>, should you need it.
     *
     * @param context   <code>ServiceContext</code> referencing the requesting
    ...
    public static boolean responseReady(NavuContext context, Cdb cdb,
        ...
        NavuContainer transPool =
            ...
            list(ipaddressAllocator._ip_address_pool_).
            elem(poolName);
        ...
        public void run() {
            try {
                 ...
                 */
                    ...
                    NavuContainer base = new NavuContainer(context);
                    NavuContainer root = base.container(resourceAllocator.hash);
                    NavuContainer resources = root.container(resourceAllocator.
                                    _resource_pools_);
